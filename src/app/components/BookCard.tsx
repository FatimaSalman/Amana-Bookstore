...
const BookCard: React.FC<BookCardProps> = ({ book, onAddToCart }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden transform hover:-translate-y-1 transition-transform duration-300">
      ...
      <div className="p-4">
        <Link href={`/book/${book.id}`} className="block cursor-pointer">
          <h3 className="text-lg font-semibold text-gray-800 truncate hover:text-yellow-600 transition-colors duration-200">{book.title}</h3>
        ...
        <div className="mt-2">
          {book.genre.slice(0, 2).map((g) => (
            <span key={g} className="inline-block bg-gray-200 rounded-full px-2 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">
              {g}
            </span>
          ))}
          {book.genre.length > 2 && (
